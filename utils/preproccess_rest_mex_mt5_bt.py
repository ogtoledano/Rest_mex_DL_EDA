...
def build_dataset_and_dict():
    ...
    for i in range(df.shape[0]):
        ...
        dictionary_data['y'].append((str(df.iloc[i, 2]-1), str(atractions[df.iloc[i, 3]])))

    X_train, X_dev, y_train, y_dev = train_test_split(dictionary_data['X'], dictionary_data['y'], test_size=0.20,
                                                      random_state=142)
    ...
    rslt_df_4 = df.loc[df['y'].isin([('4',str(attr)) for attr in atractions.values()])]
    count_major_class = rslt_df_4.shape[0]

    augmentation = {'0': 430,
                    '1': 603,
                    '2': 1669,
                    '3': 4720}# 421,591,1624,4557

    first_model_tkn, first_model = get_mt_model('es-en', device)
    second_model_tkn, second_model = get_mt_model('en-es', device)

    for i in ['0', '1', '2', '3']:
        rslt_df = df.loc[df['y'].isin([(i,str(attr)) for attr in atractions.values()])]
    ...
    torch.save(stt_train, wdir + "/datasets/dataset_train_sst_mt5_norm")
    torch.save(stt_train_bt, wdir + "/datasets/dataset_train_stt_mt5_bt")
    torch.save(stt_train_lemm, wdir + "/datasets/dataset_train_stt_mt5_lemmatized")
    torch.save(stt_train_bt_lemm, wdir + "/datasets/dataset_train_stt_mt5_bt_lemm")

    torch.save(stt_test, wdir + "/datasets/dataset_dev_stt_mt5_norm")
    torch.save(stt_test_lemm, wdir + "/datasets/dataset_dev_stt_mt5_lemmatized")
